// # settings
//
// Validates that ~/.claude/settings.json (and settings.local.json if present) exist and
// contain valid configuration. It also surfaces non-default overrides as advisory
// findings so operators can review unexpected changes.
//
// # Findings
//
...
//
// SETTINGS_OVERRIDE emits one finding per non-default key. Both settings.json and
// settings.local.json are checked when the local file exists.
//
// # Configuration
//
